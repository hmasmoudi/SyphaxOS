...
install_software() {
		  ...
		  do
			...
			if ! "$skip" ; then
				...
				if [ "$ex" -eq "1" ]; then
					...
					if ! (<"$ARCH"/etc/pacman.conf grep "SyphaxOS"); then
			...
			case "$software_menu" in
				...
				"$done_msg")
					if [ -z "$final_software" ]; then
						...
						chroot "$ARCH" pacman -Sy --print-format='%s' $(echo "$download") | awk '{s+=$1} END {print s/1024/1024}' >/tmp/size &
						...
						if (dialog --yes-button "$install" --no-button "$cancel" --yesno "\n$software_confirm_var1" "$height" 65) then
							tmpfile=$(mktemp)
						    chroot "$ARCH" pacman --noconfirm -Sy $(echo "$download") &> "$tmpfile" &
			...
			if ! "$err" ; then
				if [ -z "$software" ]; then
					...
					chroot "$ARCH" pacman -Sy --print-format='%s' $(echo "$add_software") | awk '{s+=$1} END {print s/1024/1024}' >/tmp/size &
					pid=$! pri=0.1 msg="$wait_load \n\n \Z1> \Z2pacman -Sy --print-format\Zn" load
					download_size=$(</tmp/size) ; rm /tmp/size
	...
	if ! "$pac_update" ; then
		...
		chroot "$ARCH" pacman -Sy &> /dev/null &
